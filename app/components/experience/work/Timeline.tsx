...
const TimelinePoint = ({ point, diff }: { point: WorkTimelinePoint, diff: number }) => {
  ...
  const isEducation = point.year.toLowerCase() === 'education';
  ...
  return (
    <group position={point.point} scale={isMobile ? 0.34 : 0.52}>
      ...
      <group>
        <group position={getPoint}>
          <group position={[0, -0.5, 0]}>
            <Text {...titleProps} fontSize={0.38} position={[0, -diff / 2, 0]}>
              {`${point.year}  ${point.title}`}
            </Text>
            <Text
              {...textProps}
              fontSize={isEducation ? 0.14 : 0.17}
              fillOpacity={(2 - 2 * diff) * (isEducation ? 0.7 : 1)}
              position={[0, (isEducation ? -0.74 : -0.62) - diff, 0]}>
              {point.subtitle}
            </Text>
          </group>
        </group>
