...
const ProjectTile = ({
  ...
  useEffect(() => {
    ...
    const [mesh, title, dateGroup, subtext, actions] =
    ...
    hoverAnimRef.current
      ...
      .to(subtext.position, { y: hovered ? 0.7 : 0 }, 0)
      .to(subtext, { fillOpacity: hovered ? 1 : 0, duration: 0.4 }, 0)
    ...
    if (actionLinks.length > 0) {
      hoverAnimRef.current
        .to(actions.scale, { y: hovered ? 1 : 0, x: hovered ? 1 : 0 }, 0)
        .to(actions.position, { z: hovered ? 0.3 : -1 }, 0);
    }
  }, [hovered, actionLinks.length]);
  ...
  const handleClick = (e: ThreeEvent<MouseEvent>, url: string) => {
    e.stopPropagation();
